    ...
    switch (mockPODetail.status) {
      case "DRAFT":
        return [
          ...
          { label: "发送", icon: <Send className="h-4 w-4" />, action: () => setShowSendDialog(true) },
        ]
      case "CREATED":
        return [
          { label: "编辑", icon: <Edit className="h-4 w-4" />, action: () => router.push(`/purchase/po/${params.id}/edit`) },
                    ...
                    <CardContent>
                      {!mockPODetail.emailHistory || mockPODetail.emailHistory.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                          ...
                          <p>暂无邮件发送记录</p>
                          <p className="text-xs mt-2">使用"发送"功能向供应商发送PO邮件</p>
                        </div>
                      ) : (
                        <div className="space-y-4">
                          {mockPODetail.emailHistory.map((email, index) => {
                            const statusConfig = {
                              SENT: { label: "已发送", color: "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400" },
                              FAILED: { label: "发送失败", color: "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400" },
                              PENDING: { label: "发送中", color: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400" },
                            }
                                  ...
                                  <div className="space-y-4">
                                      ...
                                      <div className="space-y-2 flex-1">
                                        ...
                                        <div className="text-sm">
                                          <div className="flex items-center gap-2 text-muted-foreground">
                                            ...
                                            <span>发送时间: {new Date(email.sentDate).toLocaleString()}</span>
                                          </div>
                                          <div className="flex items-center gap-2 text-muted-foreground mt-1">
                                            <User className="h-4 w-4" />
                                            <span>发送人: {email.sentBy}</span>
                                          </div>
                                        </div>
                                      </div>
                                      <Badge variant="outline" className="text-xs">
                                        <FileText className="h-3 w-3 mr-1" />
                                        {templateNames[email.pdfTemplate] || email.pdfTemplate}
                                      </Badge>
                                    </div>
                                    ...
                                    <div className="space-y-3 text-sm">
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                          <div className="text-xs text-muted-foreground mb-1">发件人</div>
                                          <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                          <div className="text-xs text-muted-foreground mb-1">收件人</div>
                                          <div className="flex flex-wrap gap-1">
                                      ...
                                      {email.cc && email.cc.length > 0 && (
                                        <div>
                                          <div className="text-xs text-muted-foreground mb-1">抄送</div>
                                          <div className="flex flex-wrap gap-1">
                                            {email.cc.map((cc, idx) => (
                                              <Badge key={idx} variant="outline" className="text-xs font-mono">
                                                {cc}
                                      ...
                                      <div>
                                        <div className="text-xs text-muted-foreground mb-1">邮件正文</div>

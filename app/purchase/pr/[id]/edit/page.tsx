...
import { FileText, ShoppingCart, Truck, Package, CheckCircle, Plus, Trash2, Save, Send, ArrowLeft, ShoppingBag, Settings, Clock } from "lucide-react"
...
interface PRLineItem {
  ...
  notes: string
  // 供应商信息
  preferredSupplier?: string // 首选供应商
...
const mockWarehouses = [
  { id: "WH001", name: "Main Warehouse - Los Angeles", code: "LA-MAIN" },
  { id: "WH002", name: "East Distribution Center - New York", code: "NY-EAST" },
  { id: "WH003", name: "West Fulfillment Center - Seattle", code: "SEA-WEST" },
  { id: "WH004", name: "Central Warehouse - Chicago", code: "CHI-CENTRAL" },
]

// Mock existing PR data
  ...
  lineItems: [
    {
      ...
      notes: "优先级商品",
      preferredSupplier: "Apple Inc.",
      supplierQuotePrice: 999.00,
      quoteValidDate: "2024-02-15",
      quoteAttachment: "",
      ...
      supplierInfo: {
        ...
        warehouseAddress: "1234 Main St, Los Angeles, CA 90001",
        deliveryTerms: "FOB",
        paymentTerms: "NET30",
        notes: "优先供应商，质量可靠",
      },
...
export default function EditPRPage({ params }: { params: { id: string } }) {
  const { t } = useI18n()
  // Load existing PR data (in real app, fetch by params.id)
  const existingPR = mockExistingPR

  // Header form state - initialized with existing data
  const [prNumber] = React.useState(existingPR.prNo) // PR编号不可编辑
  const [department, setDepartment] = React.useState(existingPR.department)
  const [requester, setRequester] = React.useState(existingPR.requester)
  const [prType, setPrType] = React.useState(existingPR.prType)
  const [priority, setPriority] = React.useState(existingPR.priority)
  const [budgetProject, setBudgetProject] = React.useState(existingPR.budgetProject)
  const [notes, setNotes] = React.useState(existingPR.notes)
  
  // 交付要求信息
  const [deliveryRequirements, setDeliveryRequirements] = React.useState({
    targetWarehouse: existingPR.defaultWarehouse || "",
    expectedDeliveryDate: existingPR.expectedDeliveryDate || "",
    latestShipDate: existingPR.latestShipDate || "", // 最晚发货日期
  ...
  // Line items state - initialized with existing data
  const [lineItems, setLineItems] = React.useState<PRLineItem[]>(existingPR.lineItems)
  const [nextLineNo, setNextLineNo] = React.useState(Math.max(...existingPR.lineItems.map(item => item.lineNo)) + 1)
  ...
  const handleProductsSelected = (products: Product[]) => {
    const defaultTaxRate = 13 // 默认税率13%
  ...
  // 更新供应商信息
  const handleSupplierInfoChange = (itemId: string, supplierInfo: any) => {
    updateLineItem(itemId, "supplierInfo", supplierInfo)
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-bold tracking-tight">{existingPR.prNo}</h1>
                <Badge className="bg-gray-100 text-gray-800">{t('DRAFT')}</Badge>
              </div>
              <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                <div className="flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {t('created')}: 2024-01-10 09:30:00
                </div>
                <div className="flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {t('updated')}: 2024-01-12 14:20:00
                </div>
              </div>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => console.log("Save as draft")} className="min-w-[80px]">
              <Save className="mr-2 h-4 w-4" />
              {t('saveDraft')}
            </Button>
            <Button variant="outline" size="sm" onClick={handleSave} className="min-w-[60px]">
              <Save className="mr-2 h-4 w-4" />
              {t('save')}
            </Button>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="targetWarehouse">{t('targetWarehouseLabel')} *</Label>
                  <Select 
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t('selectTargetWarehouseLabel')} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <Label htmlFor="expectedDeliveryDate">{t('expectedDeliveryTimeLabel')} *</Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label htmlFor="latestShipDate">{t('latestShipDateLabel')} *</Label>
                  <Input
                    id="latestShipDate"
                    type="date"
                ...
                <div className="space-y-2">
                  <Label htmlFor="contactPerson">{t('shippingContactPersonLabel')} *</Label>
                  <Input
                    id="contactPerson"
              ...
              {/* 收货地址 - 美国格式 */}
              <div className="space-y-4">
                <Label className="text-base font-medium">{t('shippingAddressLabel')} *</Label>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="state">{t('stateProvinceLabel')} *</Label>
                    <Select 
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="zipCode">{t('zipCodeLabel')} *</Label>
                    <Input
                      id="zipCode"
              ...
              {/* 交付备注 */}
              <div className="space-y-2">
                <Label htmlFor="deliveryNotes">{t('deliveryNotesLabel')}</Label>
                <Textarea
                  id="deliveryNotes"
            ...
            {showBatchSettings && (
              <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-medium text-blue-800">{t('batchSettingsLabel')}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label>{t('batchSetCurrencyLabel2')}</Label>
                    <Select 
                  ...
                  <div className="space-y-2">
                    <Label>{t('batchSetTaxRateLabel2')}</Label>
                    <Input
                      type="number"
                      min="0"
                      max="100"
                  ...
                  <div className="space-y-2">
                    <Label>{t('batchSetSupplierLabel2')}</Label>
                    <Input
